        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {DAYS.map((day) => {
            ...
            return (
              <div
                key={day}
                className={`rounded-xl border p-4 ${isToday ? 'border-teal-400 bg-teal-50' : 'border-gray-200 bg-white'}`}
              >
                <h2 className={`mb-3 text-sm font-semibold ${isToday ? 'text-teal-700' : 'text-gray-800'}`}>
                  ...
                  <div className="space-y-2">
                    {items.map((item) => (
                      <div key={item.id} className="rounded-lg bg-white px-3 py-2 shadow-sm">
                        ...
                        <p className="text-xs text-gray-500">{item.teacher?.name ?? 'TBA'}</p>
                        <p className="text-xs text-gray-500">{item.time} &middot; {item.room ?? 'TBA'}</p>
                      </div>
                    ))}
                  </div>
                )}
