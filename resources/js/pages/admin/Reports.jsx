  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400">Download PDF reports for the school.</p>
      </div>

      <div className="max-w-xs">
        <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-slate-300">Year Level Filter (optional)</label>
        <select value={yearLevel} onChange={(e) => setYearLevel(e.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {REPORT_TYPES.map((report) => (
          <div key={report.type} className="flex flex-col justify-between rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
            <div>
              <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200">
                <FileText size={18} />
              </span>
              <p className="mt-3 font-medium text-gray-900 dark:text-white">{report.label}</p>
              <p className="mt-1 text-xs text-gray-500 dark:text-slate-400">{report.description}</p>
            </div>
            <button
              type="button"
