    ...
    <div className="space-y-4">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Create and manage school events.</p>
        </div>
          ...
          <div key={event.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white">
            <div className="flex items-center justify-between px-4 py-3">
              <button type="button" onClick={() => toggleExpand(event)} className="flex flex-1 items-center justify-between text-left">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{formatDate(event.date)} &middot; {event.attendance_count} attendee(s)</p>
                </div>
              </button>
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => toggleExpand(event)} className="rounded-lg p-1.5 text-gray-500 hover:bg-gray-100">
            ...
            {expanded === event.id && (
              <div className="border-t border-gray-100 px-4 py-3">
                ...
                {attendanceLoading && !attendance[event.id] && <p className="text-sm text-gray-400">Loading attendance…</p>}
                {attendance[event.id] && attendance[event.id].length === 0 && <p className="text-sm text-gray-400">No attendees yet.</p>}
      ...
      >
        <form id="event-form" onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-600">Date</label>
            <input type="date" required value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-600">Description</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none" />
          </div>
        </form>
      </Modal>
