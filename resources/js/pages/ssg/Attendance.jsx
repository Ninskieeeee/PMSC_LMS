...
export default function Attendance() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Attendance Log</h1>
        <p className="text-sm text-gray-500 dark:text-slate-400">Full attendance history across all events.</p>
      </div>

      <div className="max-w-xs">
        <select value={eventId} onChange={(e) => setEventId(e.target.value)} className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
            ...
            {attendance.map((record) => (
              <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{record.event?.name}</td>
